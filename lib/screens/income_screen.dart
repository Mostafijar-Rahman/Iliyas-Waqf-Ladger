...
class _IncomeScreenState extends State<IncomeScreen> {
  ...
  Future<void> _loadData() async {
    ...
    final cats = await ApiService.getIncomeCategories();
    final list = await ApiService.getIncomes();
    setState(() {
      _categories = cats;
      _incomeList = list;
      _loading = false;
  ...
  List<dynamic> get _filteredList {
    return _incomeList.where((item) {
      ...
      if (_filterDateRange != null) {
        final date = DateTime.tryParse(item['income_date'] ?? '');
        ...
        children: [
            ...
            children: [
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    const Text('আয়ের তালিকা', style: _heading),
              ...
              Expanded(
                child: filtered.isEmpty
                    ? Center(
                  child: Text(
                    _hasActiveFilters ? 'ফিল্টার অনুযায়ী কিছু পাওয়া যায়নি' : 'এখনো কোনো আয় যোগ করা হয়নি',
                    style: _caption,
                  ),
                )
                    : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) => _incomeListTile(filtered[index]),
                  ),
                ),
              ),
            ],
          ...
          Positioned(
            ...
            child: FloatingActionButton.extended(
              ...
              label: const Text('নতুন আয়', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
            ),
          ),
        ],
      ),
  ...
  Widget _incomeListTile(dynamic item) {
    return InkWell(
        ...
        child: Row(
          children: [
            Container(
              width: 38,
              height: 38,
              decoration: const BoxDecoration(color: _C.goldLight, shape: BoxShape.circle),
              alignment: Alignment.center,
              child: const Icon(Icons.arrow_downward, size: 18, color: _C.gold),
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${item['category_name'] ?? ''} • ${item['income_date']}${(item['description'] ?? '').toString().isNotEmpty ? ' • ${item['description']}' : ''}',
                    style: _caption,
                    overflow: TextOverflow.ellipsis,
                  ),
                ],
    ...
    showDialog(
      ...
      builder: (context) => Dialog(
        ...
        child: ConstrainedBox(
          ...
          child: _IncomeAddForm(categories: _categories, onSaved: _loadData),
        ),
      ),
    );
  }

  // --- বিদ্যমান আয় এন্ট্রি এডিট ডায়লগ ---
  void _showEditDialog(dynamic item) {
    showDialog(
      ...
      builder: (context) => Dialog(
        ...
        child: ConstrainedBox(
          constraints: const BoxConstraints(maxWidth: 480),
          child: _IncomeAddForm(categories: _categories, onSaved: _loadData, existingItem: item),
        ),
      ),
    );
  }
...
// --- আলাদা স্টেটফুল উইজেট, ডায়লগের ভেতরে ফর্মের নিজস্ব state ---
class _IncomeAddForm extends StatefulWidget {
  final List<dynamic> categories;
  final VoidCallback onSaved;
  // দেওয়া হলে ফর্মটি "এডিট মোড"-এ চলবে, নাহলে "নতুন এন্ট্রি" মোডে
  final dynamic existingItem;

  const _IncomeAddForm({required this.categories, required this.onSaved, this.existingItem});

  bool get isEditMode => existingItem != null;

  @override
  State<_IncomeAddForm> createState() => _IncomeAddFormState();
}

class _IncomeAddFormState extends State<_IncomeAddForm> {
  ...
  void initState() {
    ...
    if (widget.isEditMode) {
      ...
      final parsedDate = DateTime.tryParse((item['income_date'] ?? '').toString());
  ...
  // বর্তমান সিলেকশন থেকে চূড়ান্ত ক্যাটাগরি বের করা
  ...
  Future<void> _submit() async {
    ...
    bool success;
    if (widget.isEditMode) {
      success = await ApiService.updateIncome({
      ...
      success = await ApiService.createIncome({
        'category_id': categoryId,
        'amount': double.tryParse(_amountController.text) ?? 0,
        'income_date': dateStr,
        'description': _descController.text,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            backgroundColor: _C.primary,
            content: Text(
              widget.isEditMode ? 'আয় সফলভাবে আপডেট হয়েছে' : 'আয় সফলভাবে সেভ হয়েছে',
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text(widget.isEditMode ? 'আয় এন্ট্রি এডিট করুন' : 'নতুন আয় এন্ট্রি', style: _heading),
              ...
              const SizedBox(height: 12),
              const Align(alignment: Alignment.centerLeft, child: Text('কোন মাধ্যমে গ্রহণ', style: _caption)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _submitting
                      ...
                      : Text(widget.isEditMode ? 'পরিবর্তন সংরক্ষণ করুন' : 'আয় সেভ করুন', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: Colors.white)),
                ),
              ),
            ],
          ),
